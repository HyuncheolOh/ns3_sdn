...
#include "ofqos/qos-controller.h"
...
main (int argc, char *argv[])
{
  ...
  NetDeviceContainer switch0Ports, switch1Ports, switch2Ports, switch3Ports, switch4Ports;
  ...
  // Connect client switch2 to client switch 3
  ...
  link = csmaHelper.Install (NodeContainer (switchNodes.Get (2), switchNodes.Get (4)));
  switch2Ports.Add (link.Get (0));
  ...
  // Connect client nodes to client switch
  for (size_t i = 2; i < clients ; i++)
    {
      link = csmaHelper.Install (NodeContainer (clientNodes.Get (i), switchNodes.Get (4)));
      clientDevices.Add (link.Get (0));
      switch4Ports.Add (link.Get (1));
    }
  ...
  ofLearningHelper->InstallController (controllerNodes.Get (0), learnCtrl);
  ofSwitchDevices.Add (ofLearningHelper->InstallSwitch (switchNodes.Get (0), switch0Ports));
  ofSwitchDevices.Add (ofLearningHelper->InstallSwitch (switchNodes.Get (1), switch1Ports));
  ofLearningHelper->CreateOpenFlowChannels ();

  // Install OpenFlow switches 2 with learning controller
  Ptr<OFSwitch13InternalHelper> ofLearningHelper2 = CreateObject<OFSwitch13InternalHelper> ();
  Ptr<OFSwitch13LearningController> learnCtrl2 = CreateObject<OFSwitch13LearningController> ();
  ofLearningHelper2->InstallController (controllerNodes.Get (1), learnCtrl2);
  ofSwitchDevices.Add (ofLearningHelper2->InstallSwitch (switchNodes.Get (2), switch2Ports));
  ofSwitchDevices.Add (ofLearningHelper2->InstallSwitch (switchNodes.Get (3), switch3Ports));
  ofSwitchDevices.Add (ofLearningHelper2->InstallSwitch (switchNodes.Get (4), switch4Ports));
  ofLearningHelper2->CreateOpenFlowChannels ();
  ...
  //cout << internetIpIfaces.GetAddress(1) << endl;
  //V4PingHelper pingHelper = V4PingHelper (internetIpIfaces.GetAddress (0));
  V4PingHelper pingHelper = V4PingHelper (internetIpIfaces.GetAddress(0));
  pingHelper.SetAttribute ("Verbose", BooleanValue (true));
 // ApplicationContainer pingApps = pingHelper.Install (clientNodes.Get (0));
  ApplicationContainer pingApps = pingHelper.Install (serverNodes.Get (0));
  pingApps.Start (Seconds (1));

  V4PingHelper pingHelper2 = V4PingHelper (internetIpIfaces.GetAddress(2));
  pingHelper2.SetAttribute ("Verbose", BooleanValue (true));
  ApplicationContainer pingApps2 = pingHelper2.Install (serverNodes.Get (1));
