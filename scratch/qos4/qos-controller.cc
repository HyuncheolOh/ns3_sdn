...
QosController::HandshakeSuccessful (Ptr<const RemoteSwitch> swtch)
{
  ...
  switch(id)
  {
	case 1:
	case 2:
		  ConfigureCoreSwitch (swtch, 1);
		  break;
	case 3:
		  ConfigureAggregationSwitch(swtch, 1);
		  break;
	case 4:
		  ConfigureAggregationSwitch(swtch, 2);
		  break;
	case 5:
	case 6:
		ConfigureAggregationSwitch(swtch, 3);
		  break;
	case 7:
...
{
  NS_LOG_FUNCTION (this << swtch);
  NS_LOG_FUNCTION (m_linkAggregation);
...
void
QosController::ConfigureAggregationSwitch (Ptr<const RemoteSwitch> swtch, int type)
{
  NS_LOG_FUNCTION (this << swtch);

  if (type == 1) 
  {
	//connected with border switch
	  if (m_linkAggregation)
	  {
		  // Configure Group #1 for aggregating links 3 and 4
	  ...
	  // Packets from input ports 3 and 4 are redirecte to port 1
	  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
			  "in_port=3 write:output=1");
	  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
			  "in_port=4 write:output=1");

	  // Packets from input port 1 are redirected to group 1
	  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
			  "in_port=1 write:group=1");
	  
	  // Packets from input port 2 are redirected to group 1
	  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
			  "in_port=2 write:group=1");
  ...
  else if (type == 2)
  {
	  ...
	  // condition can be added depends on the link connection, destination address
	  //DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
...
void
QosController::ConfigureCoreSwitch (Ptr<const RemoteSwitch> swtch, int type)
{
  NS_LOG_FUNCTION (this << swtch);
  ...
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
			  "in_port=1 write:output=4");


  
  /*
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
		  "eth_type=0x800,ip_dst=10.1.1.1 apply:output=1");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
		  "eth_type=0x800,ip_dst=10.1.1.2 apply:output=1");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
		  "eth_type=0x800,ip_dst=10.1.2.1 apply:output=2");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
		  "eth_type=0x800,ip_dst=10.1.3.1 apply:output=3");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
		  "eth_type=0x800,ip_dst=10.1.4.1 apply:output=4");
*/
  /*
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
		  "eth_type=0x806,ip_dst=10.1.1.1 apply:output=1");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
		  "eth_type=0x806,ip_dst=10.1.1.2 apply:output=1");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
		  "eth_type=0x806,ip_dst=10.1.2.1 apply:output=2");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
		  "eth_type=0x806,ip_dst=10.1.3.1 apply:output=3");
  DpctlExecute (swtch, "flow-mod cmd=add,table=0,prio=500 "
		  "eth_type=0x806,ip_dst=10.1.4.1 apply:output=4");
*/
}
ofl_err
  ...
  uint32_t xid)
{
  ...
  cout << "srcIP : " << srcIp << ", dstIP : " << dstIp << ", srcMac : " << srcMac << ", serverIp : " << serverIp << ", serverMac : " << serverMac << endl;
